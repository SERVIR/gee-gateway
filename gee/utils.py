...
def filteredImageInCHIRPSToMapId(dateFrom=None, dateTo=None):
    ...
    try:
        ...
        visParams = {'opacity': 1, 'max': 188.79177856445312,
...
def getTimeSeriesByIndex(indexName, scale, coords=[], dateFrom=None, dateTo=None, reducer=None):
    ...
    try:
        ...
        try:
            getTimeSeriesByIndex(indexName, scale * 2,
                                 coords, dateFrom, dateTo, reducer)
        except EEException as e:
            raise GEEException(sys.exc_info()[0])
    return out
...
def getLatestImageTileUrl(imageCollection, visParams):
    ...
    most_current = ee.Date(ic.first().get(
        'system:time_start')).format('YYYY-MM-dd')
    # logger.error(str(most_current.getInfo()))

    first = ic.filterDate(most_current, ee.Date(
        most_current).advance(1, "day")).mosaic()
...
def getTimeSeriesForPoint(point, dateFrom=None, dateTo=datetime.datetime.now()):
    ...
    collectionBands = map(lambda collectionDef: ee.ImageCollection(
        collectionDef['name']).select(collectionDef['bands'], bandNames), collectionBands)
...
def getTimeSeriesAssetForPoint(point, dateFrom=None, dateTo=datetime.datetime.now()):
    def sampleUsingPoint(image):
        ...
        sampledValue = image.sample(ee.Geometry.Point(
            point), 30).first().get(image.bandNames().get(0))
    ...
    try:
        ...
        croplands = ee.ImageCollection(
            "projects/servir-mekong/yearly_primitives_smoothed/cropland")
        croplands = toDict(croplands.toList(
            croplands.size()).map(sampleUsingPoint).getInfo())
        # alltimestamps = reduce(lambda x, y: x.union(y.keys()), [tcc,loss,croplands], set())
        alltimestamps = list(set().union(
            tcc.keys(), loss.keys(), croplands.keys()))
        alltimestamps.sort()
        timeseries = []
...
def getAsterMosaic(visParams={}, dateFrom=None, dateTo=None):
    ...
    try:
        def normalize(image):
            ...
            coefficients = [ee.Image(ee.Number(image.get(
                'GAIN_COEFFICIENT_' + band))).float().rename([band]) for band in bands]
            coefficients = ee.Image.cat(coefficients)
            cloudCover = ee.Image(ee.Number(image.get('CLOUDCOVER'))).float(
            ).multiply(-1).add(100).rename(['cloudCover'])
            image = image.select(bands).subtract(1).multiply(coefficients)
            image = image.select(
                bands, ['green', 'red', 'nir', 'swir1', 'swir2', 'thermal'])
    ...
    try:
        ...
        def maxNdvi(year):
            ...
            s2 = ee.ImageCollection(
                'COPERNICUS/S2').filterDate(from1, to).map(map1)
            l8 = ee.ImageCollection(
                'LANDSAT/LC08/C01/T1_TOA').filterDate(from1, to).map(map2)
            l7 = ee.ImageCollection(
                'LANDSAT/LE07/C01/T1_TOA').filterDate(from1, to).map(map3)
            l5 = ee.ImageCollection(
                'LANDSAT/LT05/C01/T1_TOA').filterDate(from1, to).map(map3)
        ...
        def landsatCollection1Mask(image):
            ...
            def is_set(types):
                """  """
                typeByValue = {'badPixels': 15, 'cloud': 16,
                               'shadow': 256, 'snow': 1024, 'cirrus': 4096}
                any_set = ee.Image(0)
                for type in types:
                    any_set = any_set.Or(image.select(
                        'BQA').bitwiseAnd(typeByValue[type]).neq(0))
                return any_set
...
VIS_743 = {"bands": ["B7", "B4", "B3"],
           "min": [-904, 151, -300], "max": [3696, 4951, 2500]}
VIS_432 = {"bands": ["B4", "B3", "B2"], "min": [
    151, -300, 50], "max": [4951, 2500, 1150]}
VIS_543 = {"bands": ["B5", "B4", "B3"],
           "min": [-804, 151, -300], "max": [3800, 4951, 2500]}
VIS_BGW = {"bands": ["B", "G", "W"], "min": [
    604, 49, -2245], "max": [5592, 3147, 843]}
...
def getImageCollection(point, year=None):
    ...
    lc8_collection = ee.ImageCollection(
        'LANDSAT/LC08/C01/T1_SR').filterBounds(aoi).select(BAND_SET['LC08'], BAND_NAMES)
    le7_collection = ee.ImageCollection(
        'LANDSAT/LE07/C01/T1_SR').filterBounds(aoi).select(BAND_SET['LE07'], BAND_NAMES)
    lt5_collection = ee.ImageCollection(
        'LANDSAT/LT05/C01/T1_SR').filterBounds(aoi).select(BAND_SET['LT05'], BAND_NAMES)
    lt4_collection = ee.ImageCollection(
        'LANDSAT/LT04/C01/T1_SR').filterBounds(aoi).select(BAND_SET['LT04'], BAND_NAMES)
...
def getLandsatImages(point, year=None):
    ...
    ids = all.toList(all.size()).map(
        lambda image: ee.Image(image).get('system:id'))
    return ids.getInfo()
...
def createChip(image, point, vis, size=255):
    ...
    doy = ee.Date(this_image.get('system:time_start')
                  ).getRelative('day', 'year').getInfo()
    ...
    box = ee.Geometry.Point(point).buffer(
        pixelSize.multiply(size / 2.0), 5).bounds(5)
    ...
    chip_url = ee.Image(image).visualize(
        **VIS_SET[vis]).unmask().getThumbURL(params)
...
def createChipXYZ(image, point, vis, size=255):
    ...
    mapid = ee.Image(image).clip(box).unmask(
    ).visualize(**VIS_SET[vis]).getMapId()

    chip_url = 'https://earthengine.googleapis.com/map/%s/{z}/{x}/{y}?token=%s' % (
    ...
    def qa(img):
        ...
        cfmask = ee.Image(img).select(['cfmask']).unmask(9) \
            .reduceRegion(reducer=ee.Reducer.first(),
                          ...
                          tileScale=16) \
            .get('cfmask')
        offset = ee.Number(cfmask).multiply(1000).add(
            img.date().getRelative('day', 'year').subtract(day).abs())
...
def getSpectralsForPoint(collection, point):
    ...
    def toValue(image):
        ...
        scaled = image.select(["B1", "B2", "B3", "B4", "B5", "B7"]).divide(
            10000).addBands(image.select(['cfmask']))
...
def getTsTimeSeriesForPoint(point):
    collection = ee.ImageCollection(
        getImageCollection(point))  # .map(parseQA2FMask)
...
def getTsTimeSeriesForPointByTargetDay(point, day, startYear=1985, endYear=None):
    ...
    images = ee.List.sequence(startYear, endYear).map(
        lambda y: collection.filterDate(ee.Date.fromYMD(
            y, 1, 1), ee.Date.fromYMD(y, 12, 31)).sort('offset').first()
    )
